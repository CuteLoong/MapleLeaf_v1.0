...
void Camera::Update()
{
    if (!Scenes::Get()->GetScene()->IsPaused()) frameID = (frameID + 1) % std::numeric_limits<uint32_t>::max();
...
void Camera::UpdateByInput()
{
    ...
    if (positionDelta == glm::vec3(0.0f) && rotationDelta == glm::vec2(0.0f) && scrollDelta == 0.0f) return;
...
void Camera::UpdateCameraInfo()
{
    ...
    prevOrthoMatrix      = orthoMatrix;
    prevViewMatrix       = viewMatrix;

    viewMatrix       = glm::lookAt(position, position + forward, up);
    projectionMatrix = glm::perspective(fieldOfView, aspectRatio, nearPlane, farPlane);
    orthoMatrix      = glm::ortho(-orthoScale, orthoScale, -orthoScale, orthoScale, nearPlane, farPlane);
...
void Camera::PushUniforms(UniformHandler& uniformObject)
{
    ...
    uniformObject.Push("ortho", orthoMatrix);
    uniformObject.Push("prevOrtho", prevOrthoMatrix);

                          ...
                          uint32_t baseArrayLayer, uint32_t layerCount)
{
    ...
    VkFilter filter =
        formatProperties.linearTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT ? VK_FILTER_LINEAR : VK_FILTER_NEAREST;
    ...
    for (uint32_t i = 1; i < mipLevels; i++) {
        ...
        vkCmdBlitImage(
            commandBuffer, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &imageBlit, filter);
...
bool Image::CopyImage(const CommandBuffer& commandBuffer, const Image& srcImage, const Image& dstImage, int srcMipLevel, int dstMipLevel)
{
    ...
    imageCopyRegion.extent.width                  = std::max(srcExtent.x >> srcMipLevel, 1u);
    imageCopyRegion.extent.height                 = std::max(srcExtent.y >> srcMipLevel, 1u);
